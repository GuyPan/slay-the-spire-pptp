...
import pptpinsts.powers.PopukoFormPower;
import pptpinsts.util.CardStats;

public class PptpPopukoForm extends BaseCard {
    public static final String ID = makeID(PptpPopukoForm.class.getSimpleName());
    ...
    private static final int UPG_POWER = 1;

    public PptpPopukoForm() {
        super(ID, info); //Pass the required information to the BaseCard constructor.
        setMagic(POWER, UPG_POWER);
    }

    @Override
    public void use(AbstractPlayer p, AbstractMonster m) {
        addToBot(new ApplyPowerAction(p,p,new PopukoFormPower(p,0)));
    }

    @Override
    public AbstractCard makeCopy() { //Optional
        return new PptpPopukoForm();
    }
